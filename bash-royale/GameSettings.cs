static class GameSettings
{
    public const int GAME_WIDTH = 28;
    public const int GAME_HEIGHT = 38; 

    public const int STARTING_ELIXIR = 5;
    public const int MAX_ELIXIR = 10;
    public const int ELIXIR_TICK_INTERVAL = 56;

    public const int KING_TOWER_HEALTH = 3200;
    public const int PRINCESS_TOWER_HEALTH = 1900;
    
    public const int TICKS_PER_SECOND = 20;
    public const int REGULATION_END_TICK = 3600;   // 3 minutes 
    public const int OVERTIME_END_TICK   = 3600 + 2400;   // 5 minutes 
    
    public const float MUSIC_VOLUME = 0.3f;
    public const float SFX_VOLUME = 0.8f;
}
